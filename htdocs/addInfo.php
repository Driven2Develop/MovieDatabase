...
input[type="submit"]:hover{
  ...
  cursor:pointer;
		...
		if(array_key_exists('save',$_POST))
		{
			...
			$movieID=$_POST['imovieID'];
			$language=$_POST['ilanguage'];
			...
			$query = "INSERT INTO movie_recommender.movietopics
						Values('$topicId','$movieID','$language', '$country')";
			...
			pg_close($dbconn);	
			header('Location: http://localhost/addedInfo.html');
		}
		?>
	<body>
		<form class="form" name="form" method="post" action="">
		<p> <label for="topicID">Topic ID:</label>
				<input name="topicID" type ="text" id="topicID"/>
			</p>
			<p> <label for="imovieID">Movie ID:</label>
				<input name="imovieID" type="text" id="imovieID"/>
			</p>
			<p> <label for="ilanguage">Language:</label>
				<input name="ilanguage" type ="text" id="ilanguage"/>
			</p>
			<p> <label for="icountry">Country:</label>
				<input name="icountry" type="text" id="icountry"/>
			</p>
			<p><input type="submit" value="Submit" name= "save" id="save" /> </p>
		</form>
	</body>
</html>	